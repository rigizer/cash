...
<html lang="ko">
		...
		<main id="main">
			...
			<section id="breadcrumbs" class="breadcrumbs">
				<div class="container">
					<ol>
						...
						<li>Statistics</li>
						<li>월간 카테고리별 지출 내역</li>
					</ol>
					<h2>월간 카테고리별 지출 내역</h2>
				</div>
			</section>
			<!-- End Breadcrumbs -->
			...
			<section class="inner-page pt-4">
				<div class="container">
					...
					<!-- Chart -->
					<div>
						<canvas id="categoryOutByMonthChartResult"></canvas>
					</div>
				</div>
			</section>
	...
	<script>
		<!-- Chart -->
		$.ajax({
			url:'${pageContext.request.contextPath}/admin/categoryOutByMonth/' + getParam("year") + '/' +  getParam("month"),
            type:'GET',
            success: function(data) {
               let chartResult = $('#categoryOutByMonthChartResult');
               let chart = new Chart(chartResult, {
                  type: 'polarArea',
                  data: {
                     datasets: [{
                   	 	data: [data.관광, data.생활, data.식비, data.의료], 
                        label: getParam("year")+'년 '+getParam("month")+'월 카테고리별 지출 내역 차트',
                        backgroundColor: [
                           	  'rgba(54, 162, 235, 0.2)',
                              'rgba(255, 99, 132, 0.2)',
                              'rgba(255, 206, 86, 0.2)',
